...
class CustomRecommendSpecificViewModel(activity: Activity, private val repository: TrackRepository) : ViewModel(), VisualTabClickHandler, ExpandClickHandler {
    ...
    val tracksFromDatabaseLiveData: LiveData<MutableList<TrackSpecificEntity>> =
        repository.allRecommendedSpecificEntityTracks.asLiveData()

    private val recommendedTracksLocal: MutableLiveData<List<TrackSpecificEntity>> by lazy {
        MutableLiveData<List<TrackSpecificEntity>>(arrayListOf())
    }
    private val userTracksAudioFeatures: MutableLiveData<List<TrackAudioFeatures>> by lazy {
    ...
    private suspend fun getMostSimilarTracksSpecific(recommendedTracksPool: MutableList<TrackAudioFeatures>, usersTracksPool : MutableList<TrackAudioFeatures>) {
        ...
        val sorted = recommendedToUserMap.map { Pair(first = it.key, second = it.value.values.sorted().take(10).sum())}.sortedBy { it.second }
