...
/**
 * SeekBar handler class
 * @property [seekBar] seekBar view component
 * @property [seekStopListener] stop listener
 * @property [endOfTrack] method to call at the end of track
 */
class TrackProgressBar(
    private val seekBar: SeekBar,
    private val seekStopListener: (Long) -> Unit,
    private val endOfTrack: () -> Unit
) {
    private val handler: Handler

    private val seekBarChangeListener = object : SeekBar.OnSeekBarChangeListener {
        override fun onProgressChanged(seekBar: SeekBar, progress: Int, fromUser: Boolean) {
            if (seekBar.max < progress + 2000){
                endOfTrack()
            }
        }
    ...
    companion object {
        private const val LOOP_DURATION = 250
    }
}