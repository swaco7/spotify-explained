...
object NetworkFeaturesGraph {
    ...
    fun getBody() : String {
        return " var color = d3.scaleLinear().domain([1,2])\n" +
                ...
                "    .attr(\"class\", \"labels\")\n" +
                "    .attr(\"font-weight\", function(d) { return d.group != 1 ? 600 : 600;})\n" +
                "    .attr(\"text-anchor\", \"middle\")\n" +
                "       .attr('y', 3);\n" +
                "    labels.text(function(d) {\n" +
                "      var doc = new DOMParser().parseFromString(\"max \" + d.id, \"text/html\");\n" +
    ...
    fun getZoomFeatures(): String {
        return "var zoom_handler = d3.zoom().scaleExtent([0.3, 5])\n" +
                ...
                "      var collide = (Math.sqrt(x * x + y * y) < (dist*0.9))\n" +
                "      if (collide) {\n" +
    ...
    fun getZoomResponsive() : String {
        return "var zoom_handler = d3.zoom()\n" +
                ...
                "\n" +
                "function updateVisual(){\n" +
                //+ (nodesGroups[d.id] == null ? 0 : nodesGroups[d.id].length * 3)
                " circles.style(\"r\", function(d) {\n" +
                "    var sum = 0\n" +
                "    var m = nodesGroups[d.id] == null ? 0 : nodesGroups[d.id].length\n" +
                "    for (i = 0; i < m; ++i) {\n" +
                "      sum += nodes[nodesGroups[d.id][i]].radius / 15\n" +
                "    }\n" +
                "    var fullRadius = d.radius + sum\n" +
                "    return (nodesCollisions.includes(nodes.indexOf(d)) ? 0 : (Math.max(fullRadius, fullRadius * 1.5 / (k / 1.5))));\n" +
                "  });\n" +
                "    innerCircle.style(\"r\", function(d) {\n" +
                "      return (nodesCollisions.includes(nodes.indexOf(d)) ? 0 : (Math.max(d.radius / 1.5, (d.radius * 1.5 / (k / 1.5)) / 1.5)));\n" +
                "    });\n" +
                "    centerCircle.style(\"r\", function(d) {\n" +
                "      return (nodesCollisions.includes(nodes.indexOf(d)) ? 0 : (Math.max(d.radius / 2.5, (d.radius * 1.5 / (k / 1.5)) / 2.5)));\n" +
                "    });\n" +
                "    labels.style(\"font-size\", function(d) {\n" +
                "      return (nodesCollisions.includes(nodes.indexOf(d)) ? 0 : (Math.max(10, 20 / (k))));\n" +
                "    })\n" +
                ...
                "      return (nodesCollisions.includes(nodes.indexOf(d)) ? 0 : (Math.max(10, 20 / (k))));\n" +
                "    })\n" +
                "    moreLabels.attr(\"transform\", function(d) {\n" +
                "      return \"translate(\" + 0 + \",\" + (Math.max(10, 15 / (k))) + \")\";\n" +
                "    })\n" +
                "\n" +
                "    labels.text(function(d) {\n" +
                "      var doc = new DOMParser().parseFromString(d.id, \"text/html\");\n" +
                ...
                "          nodesGroups[d.id] = null\n" +
                "      }\n" +
                "      return (nodesGroups[d.id] == null) ? \"\" : \"and \" + nodesGroups[d.id].length + \" more\"\n" +
                "    })\n" +
                "    link.style(\"stroke-width\", function(d) {\n" +
                "      return ((nodesCollisions.includes(nodes.indexOf(d.source)) || nodesCollisions.includes(nodes.indexOf(d.target))) ? 0 : Math.sqrt(d.value))\n" +
                "    })\n" +
                ...
                "  var dist = 0\n" +
                "  while (++i < n) {\n" +
                "    if (nodesCollisions.includes(parseInt(mKeys[i]))) {\n" +
                "      continue\n" +
                "    }\n" +
                "    var first = node\n" +
                "    var second = nodes[mKeys[i]]\n" +
                ...
                "        y = first.y - second.y;\n" +
                "        var firstSum = 0\n" +
                "        var firstLength = (nodesGroups[first.id] == null ? 0 : nodesGroups[first.id].length)\n" +
                "        for (h = 0; h < firstLength; ++h) {\n" +
                "          firstSum += nodes[nodesGroups[first.id][h]].radius / 15\n" +
                "        }\n" +
                "        var fullRadiusFirst = first.radius + firstSum\n" +
                "        var secondSum = 0\n" +
                "        var secondLength = (nodesGroups[second.id] == null ? 0 : nodesGroups[second.id].length)\n" +
                "        for (l = 0; l < secondLength; ++l) {\n" +
                "          secondSum += nodes[nodesGroups[second.id][l]].radius / 15\n" +
                "        }\n" +
                "        var fullRadiusSecond = second.radius + secondSum\n" +
                "        var dist = (Math.max(fullRadiusFirst, fullRadiusFirst * 1.5 / (k / 1.5))) + (Math.max(fullRadiusSecond, fullRadiusSecond * 1.5 / (k / 1.5))) - 1\n" +
                "\n" +
                "      /* var dist = (Math.max(first.radius, first.radius * 1.5 / (k / 1.5))) + (Math.max(second.radius, second.radius * 1.5 / (k / 1.5))) - 1 */\n" +
                "      /* var distBenev = (Math.max(first.radius, first.radius * 2 / k)) + (Math.max(second.radius, second.radius * 2 / k)) */\n" +
